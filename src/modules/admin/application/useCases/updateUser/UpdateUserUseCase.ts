...
export class UpdateUserUseCase {
  ...
  async execute(
    ...
    if (dto.approvalStatus !== undefined) {
      ...
      if (!isAdmin && !isLead) {
        throw new UnauthorizedActionError(
          "Only admins and leads can update user approval status"
        );
      }
      ...
      if (isSelfUpdate) {
        throw new UnauthorizedActionError(
          "You cannot update your own approval status"
        );
      }
      ...
      if (!Object.values(ApprovalStatus).includes(dto.approvalStatus)) {
        throw new InvalidUserDataError(
          `Invalid approval status. Must be one of: ${Object.values(
            ApprovalStatus
          ).join(", ")}`
        );
      }
    ...
    const updates = {
      ...
      firstName:
        dto.firstName !== undefined ? dto.firstName : userData.firstName,
